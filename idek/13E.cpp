...
//		cout<<endl;
//		cout<<q.size()<<endl;
		for(int i=currx-1;i>=1;i--){//lompat kiri
			if(grid[currx-1][curry]!=-1)break;
			if(grid[i][curry]==-1||dist[i][curry]!=-1)continue;
			q.push(mp(i,curry));
		...
		for(int i=curry-1;i>=1;i--){//lompat atas
			if(grid[currx][curry-1]!=-1)break;
			if(grid[currx][i]==-1||dist[currx][i]!=-1)continue;
			q.push(mp(currx,i));
